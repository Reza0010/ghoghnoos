...
import { X, Wand2, Sparkles, Download } from './icons';
...
const ImageRemixStudio: React.FC<ImageRemixStudioProps> = ({ isOpen, onClose, prompt, onSave }) => {
  ...
  const handleRemix = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        setError('ویرایش تصویر با شکست مواجه شد. لطفا دوباره تلاش کنید.');
      }
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-lg flex items-center justify-center p-4 z-50" onClick={onClose} dir="rtl">
      <div className="animate-fade-in bg-white dark:bg-dark-surface rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center p-5 border-b border-gray-200 dark:border-dark-overlay">
          <div className="flex items-center gap-3">
            <Wand2 className="w-7 h-7 text-dark-secondary" />
            <h2 className="text-2xl font-bold text-gray-800 dark:text-dark-text">استودیوی ریمیکس تصویر</h2>
          </div>
        ...
        <div className="flex-grow overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex flex-col items-center">
            <h3 className="text-lg font-semibold mb-3">تصویر اصلی</h3>
          ...
          <div className="flex flex-col items-center">
            <h3 className="text-lg font-semibold mb-3">نتیجه ریمیکس</h3>
            <div className="w-full aspect-square bg-gray-100 dark:bg-dark-bg rounded-lg shadow-md flex items-center justify-center">
              {isLoading ? (
                <div className="flex flex-col items-center text-dark-primary">
                  ...
                  <p className="mt-4">در حال پردازش جادویی...</p>
                </div>
              ) : remixedImageUrl ? (
                <img src={remixedImageUrl} alt="Remixed" className="rounded-lg w-full h-full object-contain" />
              ) : (
                <p className="text-gray-400 dark:text-dark-overlay">نتیجه اینجا نمایش داده می‌شود</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="p-5 border-t border-gray-200 dark:border-dark-overlay bg-gray-50 dark:bg-dark-surface/50 space-y-4">
            <div>
                <label htmlFor="instruction" className="block text-sm font-medium text-gray-700 dark:text-dark-subtext mb-1">دستور ویرایش را وارد کنید:</label>
                <div className="flex gap-3">
                    <input 
                        ...
                        placeholder="مثلا: یک کلاه قرمز به ربات اضافه کن"
            ...
            {remixedImageUrl && !isLoading && (
                 <div className="flex justify-end gap-3 animate-fade-in">
                    <button onClick={onClose} className="px-6 py-2 rounded-full text-gray-700 dark:text-dark-subtext bg-gray-200 dark:bg-dark-overlay hover:opacity-90 transition">بستن</button>
                    <button 
