...
const VideoStudio: React.FC<VideoStudioProps> = ({ onSave }) => {
    ...
    const loadingMessages = t('videoStudio.loadingMessages').split('|');
    ...
    const handleImageChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.files && event.target.files[0]) {
            ...
            if (file.size > MAX_FILE_SIZE) {
                alert(t('videoStudio.imageSizeError', { name: file.name }));
                return;
            }
            try {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (downloadLink) {
                ...
                throw new Error(t('videoStudio.downloadLinkError'));
            }
        } catch (err: any) {
            console.error(err);
            const errorMessage = err.message || t('videoStudio.unexpectedError');
    ...
    const handleSave = () => {
        ...
        const newPrompt: Prompt = {
            ...
            title: t('videoStudio.saveTitle', { prompt: promptText.substring(0, 30) }),
            content: promptText,
        ...
        onSave(newPrompt);
        alert(t('videoStudio.saveSuccess'));
    };
    ...
    if (!apiKeySelected) {
        return (
            <div className="p-6 h-full flex flex-col items-center justify-center text-center animate-fade-in">
                ...
                <h1 className="text-2xl font-bold mb-2">{t('videoStudio.apiKeyScreen.title')}</h1>
                <p className="max-w-md mb-4 text-gray-500 dark:text-dark-subtext">{t('videoStudio.apiKeyScreen.description')}</p>
                <p className="text-xs max-w-md mb-6 text-gray-400 dark:text-dark-subtext/70"
                    dangerouslySetInnerHTML={{ __html: t('videoStudio.apiKeyScreen.billingInfo') }}
                />
                <button onClick={handleSelectKey} className="bg-dark-primary text-white font-semibold px-6 py-3 rounded-full hover:bg-opacity-90 transition-all shadow-lg shadow-dark-primary/30">
                    {t('videoStudio.apiKeyScreen.button')}
                </button>
            </div>
        )
    }
    ...
    return (
        <div className="p-6 h-full overflow-y-auto animate-fade-in">
            <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-bold text-gray-800 dark:text-dark-text">{t('videoStudio.title')}</h1>
            </div>
            <p className="text-gray-500 dark:text-dark-subtext mb-6">{t('videoStudio.description')}</p>

             <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white dark:bg-dark-surface p-6 rounded-2xl shadow-lg space-y-6">
                    <div>
                        <h2 className="text-xl font-semibold mb-3">{t('videoStudio.promptTitle')}</h2>
                        <textarea
                            value={promptText}
                    ...
                    <div>
                        <h2 className="text-xl font-semibold mb-3">{t('videoStudio.startImageTitle')}</h2>
                         {startImage ? (
                            ...
                            <label className="aspect-video border-2 border-dashed border-gray-300 dark:border-dark-overlay rounded-lg flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 dark:hover:bg-dark-overlay/20 transition-colors">
                                ...
                                <span className="mt-2 text-sm text-center text-gray-500 dark:text-dark-subtext">{t('videoStudio.clickToUpload')}</span>
                                <input type="file" accept="image/png, image/jpeg, image/webp" className="hidden" onChange={handleImageChange} />
                            </label>
                        )}
                    </div>
                    <div>
                         <h2 className="text-xl font-semibold mb-3">{t('videoStudio.settingsTitle')}</h2>
                         <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-dark-subtext mb-1">{t('videoStudio.aspectRatioLabel')}</label>
                                <select value={aspectRatio} onChange={(e) => setAspectRatio(e.target.value as any)} className="w-full bg-gray-100 dark:bg-dark-bg rounded-lg border-transparent focus:border-dark-primary focus:ring-0">
                                    <option value="16:9">{t('videoStudio.aspectRatioLandscape')}</option>
                                    <option value="9:16">{t('videoStudio.aspectRatioPortrait')}</option>
                                </select>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-dark-subtext mb-1">{t('videoStudio.resolutionLabel')}</label>
                    ...
                    >
                        ...
                        {isLoading ? t('videoStudio.generatingButton') : t('videoStudio.generateButton')}
                    </button>
                </div>
                         ...
                         {isLoading ? (
                            <div className="flex flex-col items-center text-dark-primary p-4 text-center">
                             ...
                             <div className="text-center text-dark-danger p-4">
                                <p className="font-semibold">{t('videoStudio.errorTitle')}</p>
                            ...
                            <div className="text-center text-gray-400 dark:text-dark-subtext">
                                ...
                                <p className="mt-2 font-semibold">{t('videoStudio.resultPlaceholder')}</p>
                            </div>
                        )}
                    </div>
