...
import { Plus, Beaker, Trash2 } from './icons';
...
const PromptLab: React.FC<PromptLabProps> = ({ onSaveWinner }) => {
    ...
    const handleDeleteExperiment = (id: string) => {
        if (window.confirm(t('promptLab.deleteConfirm'))) {
    ...
    return (
        <div className="p-6 h-full overflow-y-auto animate-fade-in">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-3xl font-bold text-gray-800 dark:text-dark-text">{t('promptLab.title')}</h1>
                    <p className="text-gray-500 dark:text-dark-subtext">{t('promptLab.description')}</p>
                </div>
                <button
                ...
                >
                  ...
                  {t('promptLab.newExperimentButton')}
                </button>
            </div>
            ...
            {experiments.length === 0 ? (
                <div className="text-center text-gray-500 dark:text-dark-subtext mt-20 border-2 border-dashed border-gray-300 dark:border-dark-overlay rounded-2xl p-10">
                    ...
                    <h2 className="mt-4 text-xl font-semibold">{t('promptLab.empty.title')}</h2>
                    <p className="mt-2">{t('promptLab.empty.description')}</p>
                </div>
            ) : (
                        ...
                        <div key={exp.id} className="bg-white dark:bg-dark-surface rounded-xl shadow-md p-4 flex items-center justify-between transition-shadow hover:shadow-lg">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500 dark:text-dark-subtext">{t('promptLab.variationsCount', { count: exp.variations.length })}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className={`px-3 py-1 text-xs font-semibold rounded-full ${
                                    ...
                                    {exp.status === 'completed' ? t('promptLab.status.completed') : t('promptLab.status.running')}
                                </span>
                                <button onClick={() => setRunningExperiment(exp)} className="px-4 py-2 text-sm font-semibold text-dark-primary bg-dark-primary/10 rounded-lg hover:bg-dark-primary/20 transition">
                                    {exp.status === 'completed' ? t('promptLab.viewResultsButton') : t('promptLab.runButton')}
                                </button>
