...
const AthleteCard: React.FC<{ data: any }> = ({ data }) => {
  return (
        ...
        <div className="p-5 relative z-20">
            ...
            <div className="flex gap-4">
                <div className="w-20 h-24 bg-slate-700/50 rounded-lg border border-white/10 flex items-center justify-center overflow-hidden">
                     <span className="text-4xl filter drop-shadow-md">{data.gender === 'مرد' ? '🧔' : '👩'}</span>
                </div>
                <div className="flex-1 space-y-2">
                    <div className="flex justify-between">
                         <span className="text-xs text-slate-400">LEVEL</span>
                         <span className="text-xs font-bold text-white">{data.level || 'Unknown'}</span>
                    </div>
                    <div className="w-full h-[1px] bg-white/10"></div>
                    <div className="flex justify-between">
                         <span className="text-xs text-slate-400">GOAL</span>
                         <span className="text-xs font-bold text-amber-400">{data.goal ? data.goal.split(' ')[0] : 'Fitness'}</span>
                    </div>
                    <div className="w-full h-[1px] bg-white/10"></div>
                     <div className="flex justify-between">
...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, isGymMode = false }) => {
  ...
  // Gym Mode Styles: High Contrast, Larger Font, No Gradients/Blur
